	...
	public static void sendSMS(String phonenumber, String content) throws UnsupportedEncodingException {
		...
		try {	
			...
			if(responseCode==202) { // 정상 호출
				br = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));	// 데이터를 읽어들인다.
			} else { // 에러 발생
				br = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));
			}
			
			String inputLine;
			StringBuffer response = new StringBuffer();    // stringBuffer 자료형은 append를 이용해 문자열을 계속 추가할 수 있다.
